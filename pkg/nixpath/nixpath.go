...
func Validate(s string) error {
	if len(s) < nameOffset+1 {
		return fmt.Errorf("invalid path length %d for path %v", len(s), s)
	}

	if s[:len(StoreDir)] != StoreDir {
		return fmt.Errorf("Mismatching store path prefix for path %v", s)
	}

	if err := nixbase32.ValidateString(s[hashOffset : hashOffset+encodedPathHashSize]); err != nil {
		return fmt.Errorf("Error validating path nixbase32 %v: %v", err, s)
	}
	...
	for _, c := range s[nameOffset:] {
		if (c < 'a' || c > 'z') && (c < 'A' || c > 'Z') && (c < '0' || c > '9') {
			...
			return fmt.Errorf("Invalid character in path: %v", s)
		}
	}
